...
public class Iti40RequestGenerator {
    private static final Logger log = LoggerFactory.getLogger(Iti40RequestGenerator.class);

    private final MagProps magProps;

    public Iti40RequestGenerator(final MagProps magProps) {
        this.magProps = magProps;
    }

    private String BASE_MSG() {
        return "<env:Envelope xmlns:env=\"http://www.w3.org/2003/05/soap-envelope\">\n" +
                ...
                "               <wsa:Address>" + this.magProps.getBaseUrl() + "</wsa:Address>\n" +
    ...
    public SOAPMessage buildAssertion(@Body AssertionRequest request)
            throws SOAPException, IOException, XMLStreamException, AuthException {
        ...
        SOAPMessage message = factory.createMessage(new MimeHeaders(),
                                                    new ByteArrayInputStream(BASE_MSG().getBytes(StandardCharsets.UTF_8)));

        message.getSOAPHeader().addChildElement("MessageID", "wsa", "http://www.w3.org/2005/08/addressing").addTextNode(
        ...
        if (token instanceof String) {
            ...
            message.getSOAPHeader().addChildElement("Security",
                                                    "wsse",
                                                    "http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-secext-1.0.xsd").appendChild(
                    node);
        } else {
        ...
        SOAPElement claims = (SOAPElement) message.getSOAPBody().getElementsByTagNameNS(
                "http://docs.oasis-open.org/ws-sx/ws-trust/200512",
                "Claims").item(0);
        if (request.getResourceId() != null) {
            addResourceId(claims,
                          request.getResourceId()); // format "761337610435200998^^^&2.16.756.5.30.1.127.3.10.3&ISO"
        }
